	...
	for (int iMaterial = 0; iMaterial < m_Materials->size(); iMaterial++)
	{
		...
		if (pMaterial->strTexture[0])
		{
			...
			if (!bFound)
			{
				...
				wstr = wstr.substr(0, size + 1) + pMaterial->strTexture;
				V_RETURN(DXUTFindDXSDKMediaFileCch(str, MAX_PATH, wstr.c_str()));
				string str;
				str.assign(wstr.begin(), wstr.end());
				pMaterial->pTexture = IMAGEMANAGER->AddTexture(str, str)->tex;
			}
		}
	}
...
HRESULT CMeshLoader::LoadGeometryFromOBJ(const WCHAR * strFileName)
{
	...
	for (; ; )
	{
		...
		else if (0 == wcscmp(strCommand, L"vt"))
		{
			...
			float u, v;
			InFile >> u >> v;
			TexCoords.Add(D3DXVECTOR2(u, 1 - v));
		}
...
HRESULT CMeshLoader::LoadMaterialsFromMTL(const WCHAR * strFileName)
{
	...
	vector<Material*>* temp = MESHMANAGER->AddMaterial(wstr, m_Materials);
	...
	for (; ; )
	{
		...
		else if (0 == wcscmp(strCommand, L"map_Kd"))
		{
			...
			WCHAR ctr[128];
			InFile >> ctr;
			pMaterial->strTexture = ctr;
			int size = pMaterial->strTexture.rfind(L".");
			pMaterial->strTexture = pMaterial->strTexture.substr(0, size + 1) + L"png";
		}
